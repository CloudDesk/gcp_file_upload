...
export const buildInStoreInvoiceHtml = (invoice: any) => {
  ...
  const companyName = firstText(invoice?.companyname, "Revo365Global Private Limited");
  const companyAddress = firstText(invoice?.companyaddress);
  const companyGstin = firstText(invoice?.gstnumber);
  const companyPan = firstText(invoice?.companypan, companyPanFromGstin(companyGstin));
    ...
    <body>
      <main class="invoice">
        ...
        <table class="meta">
          ...
          <thead><tr><th>CUSTOMER NAME</th><th>PLACE OF SUPPLY</th><th>INVOICE NO</th><th>DATE</th></tr></thead>
          <tbody><tr>
            ...
            <td class="invoice-value">${escapeHtml(invoice?.invoicenumber)}</td>
            <td class="invoice-value">${escapeHtml(formatInvoiceDate(invoice?.invoicedate ?? invoice?.createddate))}</td>
          </tr></tbody>
        </table>
        <table class="items">
        ...
        <section class="closing-section">
          ...
          <footer class="footer">
            <div class="bank">
              ...
              ${invoice?.odaccountnumber ? `OD Acc: ${escapeHtml(invoice.odaccountnumber)}<br />` : ""}
              ${invoice?.ifsc ? `IFSC Code: ${escapeHtml(invoice.ifsc)}<br />` : ""}
              ${invoice?.branch ? `Branch: ${escapeHtml(invoice.branch)}` : ""}
            </div>
...
export const renderInStoreInvoicePdf = async (invoice: any): Promise<Buffer> => {
  ...
  try {
    ...
    await page.setContent(buildInStoreInvoiceHtml(invoice), { waitUntil: "networkidle0" });
