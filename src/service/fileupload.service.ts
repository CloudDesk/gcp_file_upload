...
const parseInvoiceSection = (value: any) => {
...
const getSectionRoundOff = (total: any) => {
  const numericTotal = Number(total);
  if (!Number.isFinite(numericTotal)) return "0.00";
  const adjustment = Math.round(
    (Math.round(numericTotal) - numericTotal + Number.EPSILON) * 100
  ) / 100;
  return adjustment.toFixed(2);
};

const prepareServiceInvoiceRoundOffFields = (invoice: any) => {
...
const hasCustomerGstNumber = (value: any) => {
  const normalized = String(value ?? "").trim().toLowerCase();
  return Boolean(normalized && normalized !== "-" && normalized !== "n/a" && normalized !== "null");
};

const prepareServiceInvoiceCustomerGstVisibility = (invoice: any) => {
